  ...
  return (
    ...
    >
      ...
      <div className="mb-4">
        <Link
          href="/races"
          className="focus-ring pressable inline-flex min-h-11 items-center gap-2 rounded-full border border-[var(--brand-soft-strong)] bg-white px-4 py-2 text-sm font-semibold text-slate-900 shadow-sm hover:border-[var(--brand)] hover:text-[var(--brand-strong)]"
        >
            ...
            <div className="mt-4 rounded-[1.25rem] bg-white/8 p-4 ring-1 ring-white/10 backdrop-blur">
              ...
              <div className="mt-3 flex flex-wrap gap-3 text-sm font-semibold text-slate-100">
                ...
                {calendarDownloadPath ? (
                  <a href={calendarDownloadPath} className="focus-ring inline-flex min-h-11 items-center rounded-full border border-white/15 bg-white/8 px-4 py-2 underline-offset-4 pressable hover:bg-white/12">
                    ICS 저장
                  </a>
                ) : null}
              </div>
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {relatedRaces.map((item) => (
                <Link
                  ...
                  className="interactive-card group relative overflow-hidden rounded-[1.25rem] border border-black/5 p-4 hover:-translate-y-0.5"
                >
                  <LinkPendingOverlay label="추천 대회 여는 중…" />
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="tabular-nums text-[11px] font-semibold text-[var(--brand)]">
                        {formatRaceDate(item.eventDate, item.eventDateLabel)}
                      </p>
                      <h4 className="text-balance mt-1 line-clamp-2 text-sm font-semibold text-slate-950">{item.title}</h4>
                    </div>
